...
int main() {
    const char *archivoCsv = "correos.csv";
    ...
    FILE *archivoSalida = fopen("Resultado_correos.txt", "w");
    if (!archivoSalida) {
        perror("Error al abrir 'Resultado_correos.txt'");
        liberarMemoria(nombresPalabras, MAX_COLUMNS);
        return 1;
    }
    ...
    printf("El conteo se paso correctamente al .txt\n");
